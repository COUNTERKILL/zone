...
<?php
	if($Items!='')
	{
		...
		{
			echo '<li><img class="image" src="/images/'.($value['category']).'/'.($value['image']).'" alt="."/>';
			...
			echo '</div></div>';
			echo '<div class="links"><a class="button" href="/game/bar/shop/cell/?id='.($value['inventory_id']).'&category='.($value['category']).'">Продать</a></div></li>';
		}
		echo '</ul>';
	}
	else
